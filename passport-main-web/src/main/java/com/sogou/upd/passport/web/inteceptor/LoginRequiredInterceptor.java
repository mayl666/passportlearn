 ...
 * 查看用户是否已经登录
 * 如果用户登录执行方法
 * 如果用户未登录，拦截方法执行，根据配置的{@link com.sogou.upd.passport.web.annotation.ResponseResultType}做相应处理
...
public class LoginRequiredInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws java.lang.Exception {
        ...
        //未登陆，根据配置返回
        ResponseResultType resultType = loginRequired.resultType();
        switch (resultType) {
            case json:
            case xml:
            case txt:
                String msg = String.format(resultType.value(), loginRequired.message());
                response.setContentType(HttpConstant.ContentType.HTML_TEXT + ";charset=UTF-8");
