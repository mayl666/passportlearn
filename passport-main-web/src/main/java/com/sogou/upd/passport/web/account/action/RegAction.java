...
public class RegAction extends BaseController {
    ...
    public void activeEmail(HttpServletRequest request, HttpServletResponse response, ActiveEmailParams activeParams, Model model)
        ...
        if (!Strings.isNullOrEmpty(validateResult)) {
            result.setCode(ErrorUtil.ERR_CODE_COM_REQURIE);
            //todo 返回到错误页面
            response.sendRedirect(CommonConstant.EMAIL_REG_FAILED_URL + "?code=" + result.getCode() + "&message=" + ErrorUtil.getERR_CODE_MSG(result.getCode()));
        }
        //验证client_id
        ...
        if (result.isSuccess()) {
            ...
            if (result.isSuccess()) {
                ...
                response.sendRedirect(activeParams.getRu() + "?client_id=" + clientId + "&ru=" + activeParams.getRu());
            }
        }
        response.sendRedirect(CommonConstant.EMAIL_REG_FAILED_URL + "?code=" + result.getCode() + "&message=" + ErrorUtil.getERR_CODE_MSG(result.getCode()));
