...
public class ResetPwdAction {
    ...
    public String sendEmailRegResetPwd(BaseAccountParams params, Model model) throws Exception {
        ...
        int clientId = Integer.parseInt(params.getClient_id());
        result = secureManager.sendEmailResetPwdByPassportId(passportId, clientId, true);
        model.addAttribute("data", result.toString());
        if (result.isSuccess()) {
            return ""; // TODO:
        }
    ...
    public String resetPwd(AccountPwdScodeParams params) throws Exception {
        ...
        result = secureManager.resetPasswordByEmail(passportId, clientId, password, scode);
        return "";
    }
}
