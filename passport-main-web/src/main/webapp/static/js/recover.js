...
define('recover', ['./common', './form', './tpl', './utils'], function(common, form, ursa, utils) {
    ...
    var pagefunc = {
        ...
        type: function(data) {
            ...
            var tpl = $('#Target');
            if (tpl.size()&&c) {
                tpl.parent().html(ursa.render(tpl.html(), {
                    verify: verify,
                    uname: data.userid
                }));
            } else if(tpl.size()) {
                tpl.parent().html("<p class='ptitle'>没有可以用于找回密码的方式</p>");
            }
         ...
         * @return {[type]}            [description]
         */
        index: function(data, statusText) {
        ...
        email: function(data) {
            ...
            $("#sendEmailBtn").click(function(e) {
                if(sent)return;
                var oldText=$(this).html(),selfBtn=$(this);
                $.ajax({
    ...
    return {
        init: function(type) {
            ...
            pagefunc[type] && pagefunc[type](data, statusText);
        }
    };
});