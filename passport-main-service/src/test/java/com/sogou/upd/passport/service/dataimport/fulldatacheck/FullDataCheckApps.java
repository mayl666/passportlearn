 ...
 * Time: 下午7:48
 */
public class FullDataCheckApps extends RecursiveTask<List<String>> {
    ...
    protected List<String> compute() {
        ...
        List<String> differenceLists = Lists.newLinkedList();

        StopWatch watch = new StopWatch();
        watch.start();
        try {
            for (int i = 1; i < 2; i++) {
                String filePath = BASE_FILE_PATH + "full_check_data_split_" + i + ".txt";
            ...
            for (RecursiveTask<List<String>> task : forks) {
                try {
                    ...
                    differenceLists.addAll(task.get());
