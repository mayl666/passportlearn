...
public class StringUtil {
	...
	public static String urlEncodeUTF8(String params) throws UnsupportedEncodingException {
		String en = URLEncoder.encode(params, CommonParameters.UTF8);
		en = en.replace("+", "%20");
		en = en.replace("*", "%2A");
		return en;
	}
	 ...
	 * @throws java.io.UnsupportedEncodingException
	 */
	public static String encode(String params, String encoding) throws UnsupportedEncodingException {
		String en = URLEncoder.encode(params, encoding != null ? encoding : CommonParameters.UTF8);
		en = en.replace("+", "%20");
		en = en.replace("*", "%2A");
		return en;
	}
	...
	public static String strToUTF8(String str) throws UnsupportedEncodingException {
		...
		if (!StringUtils.isEmpty(str)) {
			byte[] bytes = str.getBytes(CommonParameters.UTF8);
			s = new String(bytes, CommonParameters.UTF8);
		}
		return s;
	}
}
